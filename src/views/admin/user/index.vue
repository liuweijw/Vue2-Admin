<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-button v-if="sys_user_add" class="filter-item" style="margin-left: 10px;" type="primary" icon="edit" v-waves @click="handleAdd()">添加</el-button>
    </div>
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="数据加载中..." border fit
      ...
      <el-table-column align="center" label="操作">
        <template slot-scope="scope">
          <el-button v-if="sys_user_upd" size="small" type="success" @click="formEdit(scope.row)" v-waves>编辑</el-button>
          <el-button v-if="sys_user_del" size="small" type="danger" @click="handleDelete(scope.row)" v-waves>删除</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form :model="editForm" :rules="editFormRules" ref="editForm" label-width="100px">
        ...
        <el-form-item label="角色" prop="roleId">
          <el-select class="filter-item" v-model="editForm.roleId" placeholder="请选择">
            <el-option v-for="item in editRolesOptions" :key="item.roleId" :label="item.roleDesc" :value="item.roleId" :disabled="editIsDisabled[item.delFlag]">
        ...
        <el-form-item label="状态" v-if="dialogStatus == 'update' && sys_user_del " prop="delFlag" >
          <el-select class="filter-item" v-model="editForm.delFlag" placeholder="请选择" clearable filterable>
...
export default {
  ...
  data() {
    return {
      ...
      editFormRules: {
        ...
        delFlag: [{ required: true, message: '请选择状态', trigger: 'blur' }]
      },
      editRolesOptions: [],
      editIsDisabled: {
        0: false, 1: true
  ...
  created() {
    this.getUserList()
    // 设置权限，后续将采用动态方式
    this.sys_user_upd = this.permissions['t_sys_user_upd']
    this.sys_user_del = this.permissions['t_sys_user_del']
    this.sys_user_add = this.permissions['t_sys_user_add']
  },
  methods: {
    getUserList() {
    ...
    formEdit(row) { // 编辑
      ...
      fetchUserByUserId(row.userId).then(res => {
        ...
        this.editForm.delFlag = res.data.delFlag
