<template>
  <div class="crud-container pull-auto">
    <el-table :data="tableData"
              ...
              ref="table"
              :style="{width:setPx(tableOption.width,'99.5%')}"
      ...
      <template v-for="(column,index) in tableOption.column">
        <el-table-column :width="column.width"
          ...
          <template slot-scope="scope">
            ...
            <span v-else-if="!column.overHidden"
                  v-html="detail(scope.row,column)"></span>
            <el-popover v-else
                        trigger="hover"
                        placement="top">
      ...
      <el-table-column label="操作"
                       :width="tableOption.menuWidth?tableOption.menuWidth:240"
                       v-if="tableOption.menu==undefined?true:tableOption.menu">
        <template slot-scope="scope">
          <template v-if="tableOption.menu!=undefined?tableOption.menu:true">
            <el-button type="primary"
                       ...
                       v-if="tableOption.editBtn==undefined?true:tableOption.editBtn">编 辑</el-button>
            <el-button type="danger"
                       icon="el-icon-delete"
                       size="small"
                       @click="rowDel(scope.row,scope.$index)"
                       v-if="tableOption.delBtn==undefined?true:tableOption.delBtn">删 除</el-button>
          </template>
          <slot :row="scope.row"
                name="menu"
    ...
    <el-pagination v-if="tableOption.page!=undefined?tableOption.page:true"
            ...
            <el-col :span="column.span||12"
              ...
              <el-form-item :label="column.label"
                ...
                <component :is="getComponent(column.type)"
                           ...
                           :dic="setDic(column.dicData,DIC[column.dicData])"
                           :disabled="boxType==0?(column.addDisabled!=undefined?column.addDisabled:column.disabled):column.disabled"></component>
              </el-form-item>
            </el-col>
          </template>
        </el-row>
